...
def main():
    ...
    print(f"[OK] Loaded API key from .env (ends with: ...{api_key[-4:]})")
    ...
    # Get OpenAI client and build replacement provider
    client = create_client()
    
    # Load prompt template
    template_path = Path("src/templates/prompt_template.txt")
    ...
    print("[OK] Created replacement provider")
        ...
        if filename:
            ...
            if filename.endswith('.pdf'):
                ...
                try:
                    ...
                    print(f"\n   PII CHECK IN PDF:")
                    pdf_pii_checks = [
                        ("Paul Wayne", "Should be replaced"),
                        ("Person B", "Should be present (replacement)"),
                        ("email@email.com", "Should be replaced"),
                        ("personc@example.com", "Should be present (replacement)"),
                        ("Top Media Studies", "Should be replaced"),
                        ("Company X", "Should be present (replacement)"),
                    ]
